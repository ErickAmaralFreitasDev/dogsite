import React from "react";

interface SairIconProps extends React.SVGProps<SVGSVGElement> {
  color?: string;
  size?: number | string;
  arrowColor?: string; // Cor separada para a seta
  doorColor?: string;  // Cor separada para a porta
  thickness?: number;  // Espessura das linhas
}

const SairIcon: React.FC<SairIconProps> = ({
  color = "#333",       // Cor padrão (se não usar as específicas)
  size = "28",
  arrowColor = color,   // Herda a cor principal se não definida
  doorColor = color,    // Herda a cor principal se não definida
  thickness = 1,        // Espessura relativa
  ...props
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 28 28"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      {/* Seta (parte 1) - Customizável separadamente */}
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M19.2929 9.29289C19.6834 8.90237 20.3166 8.90237 20.7071 9.29289L24.7071 13.2929C25.0976 13.6834 25.0976 14.3166 24.7071 14.7071L20.7071 18.7071C20.3166 19.0976 19.6834 19.0976 19.2929 18.7071C18.9024 18.3166 18.9024 17.6834 19.2929 17.2929L21.5858 15H12C11.4477 15 11 14.5523 11 14C11 13.4477 11.4477 13 12 13H21.5858L19.2929 10.7071C18.9024 10.3166 18.9024 9.68342 19.2929 9.29289Z"
        fill={arrowColor}
        stroke={arrowColor}
        strokeWidth={thickness}
      />
      
      {/* Porta (parte 2) - Customizável separadamente */}
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M5 6C5 5.44772 5.44772 5 6 5H15C15.5523 5 16 5.44772 16 6V10C16 10.5523 15.5523 11 15 11C14.4477 11 14 10.5523 14 10V7H7V21H14V18C14 17.4477 14.4477 17 15 17C15.5523 17 16 17.4477 16 18V22C16 22.5523 15.5523 23 15 23H6C5.44772 23 5 22.5523 5 22V6Z"
        fill={doorColor}
        stroke={doorColor}
        strokeWidth={thickness}
      />
    </svg>
  );
};

export default SairIcon;